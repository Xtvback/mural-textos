...
import { PlusCircle, Image as ImageIcon, Calendar, Search, Loader2, Trash2, Edit } from "lucide-react"
      ...
      <main className="max-w-6xl mx-auto">
        {mostrarForm && (
          <form onSubmit={enviarTexto} className="mb-12 bg-white p-8 border border-gray-100 shadow-xl max-w-2xl mx-auto animate-in fade-in slide-in-from-top-4 duration-500">
            ...
            <button type="submit" disabled={carregando} className="w-full bg-gray-900 text-white py-4 rounded hover:bg-black transition flex justify-center items-center gap-2 uppercase tracking-widest text-xs disabled:bg-gray-400">
          ...
          {textosFiltrados.map((t) => (
            <article key={t.id} className="bg-white border border-gray-100 shadow-sm hover:shadow-xl transition-all duration-500 flex flex-col group relative">
              <div className="absolute top-4 right-4 flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity z-10">
                <button onClick={() => prepararEdicao(t)} className="p-2 bg-white/90 rounded-full shadow-sm hover:bg-white text-gray-600" title="Editar">
                  <Edit size={16} />
                </button>
                <button onClick={() => apagarTexto(t.id)} className="p-2 bg-white/90 rounded-full shadow-sm hover:bg-red-50 text-red-500" title="Eliminar">
                  <Trash2 size={16} />
                </button>
              </div>
              {t.imagemUrl && (
