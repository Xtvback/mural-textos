    ...
    <div className="min-h-screen bg-[#fcfaf7] text-[#2d2a26] font-sans selection:bg-[#8c7851] selection:text-white transition-all duration-500">
      ...
      <header className="pt-40 pb-20 px-8 border-b border-[#2d2a26]/5">
        <div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-2 items-end gap-12">
          <div>
            <span className="text-[9px] uppercase tracking-[0.6em] text-[#8c7851] font-bold block mb-4">Arquivos da Memória</span>
            <h1 className="text-3xl md:text-5xl font-serif italic tracking-tight leading-tight text-[#2d2a26]">
              Onde o Silêncio não Chega
            </h1>
          </div>
          <div className="pb-2 text-right">
            <p className="text-base md:text-lg font-serif italic text-gray-400 leading-relaxed pl-6">
              A curadoria literária de André M. Fernandes.
            </p>
          </div>
        </div>
      </header>
          ...
          <div className="columns-1 md:columns-2 gap-10 space-y-10">
            {textosFiltrados.map((t) => (
              <article key={t.id} className="break-inside-avoid bg-white border border-[#2d2a26]/5 p-8 transition-all duration-700 group relative hover:border-[#8c7851]/30 hover:shadow-xl">
                ...
                {t.imagemUrl && (
                  <div className="mb-8 border border-[#2d2a26]/5">
                    <img src={t.imagemUrl} alt="" className="w-full h-auto block" />
                  </div>
                )}
                <div className="text-[9px] text-[#8c7851] uppercase tracking-[0.4em] mb-4 font-bold">{formatarDataExibicao(t.dataManual)}</div>
                <h2 className="text-xl md:text-2xl font-serif italic text-[#2d2a26] mb-8 leading-tight tracking-tight group-hover:text-[#8c7851] transition-colors">{t.titulo}</h2>
      ...
      {/* Janela de Leitura - Imagem Completa e Título Sentido */}
      {textoAberto && (
        <div className="fixed inset-0 bg-[#fcfaf7]/98 z-[110] flex justify-center items-start overflow-y-auto p-4 md:p-12 animate-in fade-in duration-500">
          <div className="max-w-2xl w-full bg-white border border-[#2d2a26]/10 p-6 md:p-12 relative shadow-2xl my-8">
            <button onClick={() => setTextoAberto(null)} className="absolute top-4 right-4 text-gray-300 hover:text-black transition-colors z-20">
              <X size={24} />
            </button>
            <header className="mb-10 text-center">
              ...
              <h1 className="text-2xl md:text-3xl font-serif italic mb-10 leading-tight tracking-tight">{textoAberto.titulo}</h1>
              {textoAberto.imagemUrl && (
                <div className="w-full mb-10 border border-[#2d2a26]/5 shadow-sm">
                  <img src={textoAberto.imagemUrl} alt="" className="w-full h-auto block" />
                </div>
              )}
            </header>
            <div className="max-w-xl mx-auto">
              <p className="text-[#4a4540] font-serif italic text-lg md:text-xl leading-[1.8] whitespace-pre-wrap">
      ...
      <footer className="bg-white border-t border-[#2d2a26]/10 py-20 px-8 mt-20">
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row justify-between items-center gap-12 text-center md:text-left">
          <div>
            <h2 className="text-xl font-serif italic text-[#2d2a26] mb-2 uppercase tracking-widest">Arquivos da Memória</h2>
            <p className="text-gray-400 font-serif italic text-sm">André M. Fernandes</p>
          </div>
          <div className="flex flex-col items-center md:items-end gap-6">
            ...
            <p className="text-[8px] uppercase tracking-[0.8em] text-gray-300">© 2026 • Todos os Direitos Reservados</p>
          </div>
        </div>
      </footer>
